...
def run_single_experiment(
    use_structural_features, aggregator, device="cpu", epochs=100, patience=20
):
    """Run a single experiment."""

    exp_name = (
        f"{'Augmented' if use_structural_features else 'Baseline'}-{aggregator.upper()}"
    )
    print("\n" + "=" * 70)
...
def main():
    ...
    aggregators = [
        "mean",
        "max",
    ]  # LSTM consumes way more memory so I'm skipping it cuz my laptop is trash :(
    epochs = 100
    patience = 20
    ...
    for aggregator in df["aggregator"].unique():
        ...
        augmented = df[
            (df["features"] == "Augmented") & (df["aggregator"] == aggregator)
        ]
